...
ALTER SESSION SET QUERY_TAG = '''{"origin":"sf_sit-is", "name":"Eutelsat AI Demo", "version":{"major":1, "minor":0},"attributes":{"is_quickstart":0, "source":"sql"}}''';

-- ============================================================================
-- Create Warehouse Early (required for EXECUTE IMMEDIATE blocks)
-- ============================================================================
CREATE WAREHOUSE IF NOT EXISTS {{ env.EVENT_WAREHOUSE | default('EUTELSAT_DEMO_WH') }}
...
GRANT USAGE ON WAREHOUSE {{ env.EVENT_WAREHOUSE | default('EUTELSAT_DEMO_WH') }} TO ROLE {{ env.EVENT_ATTENDEE_ROLE | default('TELCO_ANALYST_ROLE') }};
GRANT OPERATE ON WAREHOUSE {{ env.EVENT_WAREHOUSE | default('EUTELSAT_DEMO_WH') }} TO ROLE {{ env.EVENT_ATTENDEE_ROLE | default('TELCO_ANALYST_ROLE') }};
...
CREATE DATABASE IF NOT EXISTS {{ env.EVENT_DATABASE | default('EUTELSAT_AI_DEMO') }}
    COMMENT = 'Database for Eutelsat AI demo - Multi-orbit data, documents, and apps pre-loaded';

USE DATABASE {{ env.EVENT_DATABASE | default('EUTELSAT_AI_DEMO') }};

-- Create schemas
CREATE OR REPLACE SCHEMA {{ env.EVENT_SCHEMA | default('EUTELSAT_SCHEMA') }}
    COMMENT = 'Main schema for Eutelsat demo data tables';
...
CREATE OR REPLACE NETWORK RULE telco_web_access_rule
  ...
  COMMENT = 'Permissive network access for Eutelsat demo web scraping';
...
ALTER USER {{ env.EVENT_USER_NAME }} SET
    ...
    DEFAULT_WAREHOUSE = {{ env.EVENT_WAREHOUSE | default('EUTELSAT_DEMO_WH') }};
...
SELECT 'Account configuration complete!' AS status,
       '{{ EVENT_DATABASE | default("EUTELSAT_AI_DEMO") }}' AS database_created,
       '{{ EVENT_WAREHOUSE | default("EUTELSAT_DEMO_WH") }}' AS warehouse_created,
...
SELECT 
    ...
    '{{ EVENT_DATABASE | default("EUTELSAT_AI_DEMO") }}' as database_name,
    '{{ EVENT_WAREHOUSE | default("EUTELSAT_DEMO_WH") }}' as warehouse_name,
